...
class PPNet(nn.Module):
    def __init__(
        ...
        self.proto_layer_rf_info = proto_layer_rf_info  # TODO CHECK
    ...
    def _l2_convolution(self, x):
        """
        apply self.prototype_vectors as l2-convolution filters on input x for images
        """
        x2 = x**2 # shape (N, C, H=14, W=14) , ones shape is (P, C, H=1, W=1)
        x2_patch_sum = F.conv2d(input=x2, weight=self.ones) # shape (N, P, 14, 14)

        p2 = self.prototype_vectors**2
        p2 = torch.sum(p2, dim=(1, 2, 3))  # sum over D, H, W
    ...
    def prototype_distances(self, x):
        """
        x is the raw input for image input
        """
    ...
    def distance_2_similarity(self, distances):
        if self.prototype_activation_function == "log":
            ...
            return -distances
        elif self.prototype_activation_function == 'exp_norm':
            dist_max = self.prototype_shape[1] * self.prototype_shape[2] *\
                self.prototype_shape[3] * self.prototype_shape[4] # 4 dim cause it is 3d video
            return 1 / ((distances / dist_max) + self.epsilon)
        else:
            return -distances # TODO: check
    ...
    def set_last_layer_connection(self):
        ...
        if self.init_ll == 'class_idx':
            self.last_layer.weight = nn.Parameter(torch.sqrt(class_IDx), requires_grad=True)
        
        elif self.init_ll == 'ones':
            self.last_layer.weight = nn.Parameter(torch.ones_like(class_IDx), requires_grad=True)

    def set_last_layer_incorrect_connection(self, incorrect_strength): # this is for xprotonet
        """
        the incorrect strength will be actual strength if -0.5 then input -0.5
        """

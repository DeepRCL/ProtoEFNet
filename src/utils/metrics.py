...
class SparsityMetric(Metric):
    ...
    def update(self, prototype_activations: torch.Tensor):
        ...
        proto_norm = prototype_activations / torch.sum(prototype_activations, dim=1).unsqueeze(-1) # shape B, P

        # sort and compute cumulative sum
        sorted, indices = torch.sort(proto_norm, descending=True, dim=1) # B, P
        cumsum = torch.cumsum(sorted, dim=1) # B, P
        num_weights = torch.ge(cumsum, self.level).type(torch.uint8).argmax(dim=1) # shape B argmax returns the first index where the condition is met, aka is sparsity prototype

        # Gather results
        self.percentage_expl += torch.sum(num_weights) # scaler, sum of sparsity prototypes
        self.total += num_weights.numel() # scaler = N

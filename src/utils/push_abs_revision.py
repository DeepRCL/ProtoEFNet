...
def prototype_plot_frame(
    unnorm_img,  # Shape = (Ho, Wo, 3)
    upsampled_occ_map,  # Shape = (Ho, Wo, 3)
    prototypes_src_imgs_masked,  # Shape = (Ho, Wo, 3)
    prototypes_overlayed_imgs,  # Shape = (Ho, Wo, 3)
    occ_map_min,
    occ_map_max,
    proto_id,
    fn,
...
def prototype_plot(
    ...
    if D == 3:
        ...
        prototype_plot_frame(
            unnorm_img,  # Shape = (Ho, Wo, 3)
            upsampled_occurrence_map,  # Shape = (Ho, Wo)
            prototypes_src_imgs_masked,  # Shape = (Ho, Wo, 3)
            prototypes_overlayed_imgs,  # Shape = (Ho, Wo, 3)
        ...
        for t in range(unnorm_img.shape[0]):
            ...
            frames.append(prototype_plot_frame(
                unnorm_img[t],  # Shape = (Ho, Wo, 3)
                upsampled_occurrence_map[t],  # Shape = (Ho, Wo)
                prototypes_src_imgs_masked[t],  # Shape = (Ho, Wo, 3)
                prototypes_overlayed_imgs[t],  # Shape = (Ho, Wo, 3)
                np.amin(upsampled_occurrence_map),
                np.amax(upsampled_occurrence_map) + 1e-7,
                proto_id,
                fn,
...
def push_prototypes(
    ...
    preprocess_input_function=None,  # if needed
    ...
    for push_iter in iterator:
        ...
        gt = data_sample["target_EF_Category"].detach().cpu().numpy()  # shape (B)
        image = x.detach().cpu().numpy()  # shape (B, 3, (To), Ho, Wo)
        filename = data_sample["filename"]  # shape (B)
        ed_frames = data_sample["ed_frame"]  # shape (B)
        es_frames = data_sample["es_frame"]  # shape (B)
    ...
    es_frames = np.array(es_frames_)
    ed_frames = np.array(ed_frames_)
    ...
    prototype_data_dict = {
        ...
        "prototypes_es_frames": es_frames,
        "prototypes_ed_frames": ed_frames,
    }
    ...
    prots_heatmaps = get_heatmap(rescaled_occurrence_maps)  # shape (P, (To), Ho, Wo, 3)

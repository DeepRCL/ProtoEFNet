...
label_schemes: Dict[str, Dict[str, Union[int, float]]] = {
    ...
    "ef_4classclinical": {"severe abnormal": 0, "moderately abnormal": 1, "mildly abnormal": 2, "normal": 3},
    "ef_2class": {"normal": 0, "abnormal": 1},
}

class_labels: Dict[str, List[str]] = {
    "binary": ["Normal", "AS"],
    "ef_4classclinical": ["Severe Abnormal", "Moderately Abnormal", "Mildly Abnormal", "Normal"],
    "ef_2class": ["Normal", "Abnormal"],
}
...
def get_loader(loader_name):
    loader_lookup_table = {"ef_loader": mat_loader}
...
def categories(x, categories_dict): 
    # reject nan values
    if pd.isnull(x):
        return np.nan
    text = x.lower()

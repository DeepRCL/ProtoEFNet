...
def explain_local(
    ...
    prots_rescaled_occurrence_maps, _ = get_normalized_upsample_occurence_maps(
    ...
    for i in iterator:
        ...
        # resize, upsample, and normalize the occurrence map.  shape (P, (To), Ho, Wo)
        rescaled_occurrence_maps, _ = get_normalized_upsample_occurence_maps(occurrence_maps, upsampler)
        ...
        sorted_similarities_indices = np.argsort(similarities)[::-1]  # Sort in descending order
        sorted_similarities = similarities[sorted_similarities_indices]

        format = "gif"  # gif or mp4
        save_path = os.path.join(root_dir_for_saving, mode, "local", test_filename)
        saveVideo(test_image_raw * 255, save_path, f"test_clip_EF-{gt}", format=format, fps=10)
        ...
        for p in prot_iterator:
            ...
            if not os.path.exists(file_path):
                ...
                for t in frame_iterator:
                    ...
                    beta = softmax((similarities * fc_layer_weights) / 0.2, axis=1)  # shape (B, P), row sum is 1
                    contribution = beta[:, p]  # Directly use beta as contribution
                    
                    axs[2].title.set_text(
                        # f"Importance = {[f'{importance[c]:.2f}' for c in range(gt.shape[0])]}\n"
                        f"        Contibution       \n"
                        f"{[f'{int(contribution*100)}']}"
                    )

                    # add super title for the figure
                    fig.suptitle(
                        f"p_{p:02d}  | img_pred = {[f'{pred[c]:.2f}' for c in range(pred.shape[0])]} | gt = {gt} | prot_gt = {proto_gts[p]}",
                        fontsize=15,
                    )
            ...
            if not os.path.exists(file_path):
                ...
                for t in frame_iterator:
                    ...
                    # Convert figure to numpy array ensuring valid dtype
                    for ax in fig.axes:
                        for img in ax.get_images():
                            if img.get_array().dtype not in [np.uint8, np.float32, np.float64, np.int16]:
                                img.set_data(img.get_array().astype(np.float32))
